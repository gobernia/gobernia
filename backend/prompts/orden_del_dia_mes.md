...
3. `owner`: rol directivo responsable de PRESENTAR la información o responder por el avance (Director General, CFO, Director Comercial, etc.). El owner no sustituye al `lead_agent`, que es el consejero IA que lidera el análisis.
...
6. `kpi_ref`: KPI principal que permite evaluar el asunto, o null. `required_doc`: documento/dato que debe recibir el Consejo. `agenda_type`: `informacion|seguimiento|deliberacion|decision`. `decision_expected`: decisión, recomendación o definición que se espera producir si aplica.
